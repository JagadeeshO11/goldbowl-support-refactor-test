...
import { Headphones, Mail, Lock, ShieldCheck, ArrowRight, LifeBuoy } from 'lucide-react'
...
export function SupportSignInPage() {
  ...
  const handleLogin = (e) => {
    e.preventDefault()
    if (email && password) {
      sessionStorage.setItem('bowlSupportAuth', '1')
      sessionStorage.setItem('bowlSupportEmail', email)
  ...
  const handleDemoLogin = () => {
    sessionStorage.setItem('bowlSupportAuth', '1')
    sessionStorage.setItem('bowlSupportEmail', 'support@goldenbowl.com')
    navigate('/support/dashboard', { replace: true })
  }
